...
"""
:Author: Liuhuanghai
:Date: 2023-05-11 16:48:52
:LastEditors: Liuhuanghai
:LastEditTime: 2023-05-11 16:48:53
:Description: 评测指标
"""
import numpy as np
...
def agg_event_role_tpfpfn_stats(pred_records, gold_records, role_num):
    ...
    """
    role_tpfpfn_stats = [
        [0] * 3 for _ in range(role_num)]  # TP,FP,FN statistics
    ...
    if gold_records is None:
        ...
        if pred_records is None:  # FN
            ...
            while len(pred_records) > 0 and len(gold_records) > 0:
                ...
                def _tmp_key(gr): return sum(
                    [1 for pa, ga in zip(pred_record, gr) if pa == ga]
                )
                best_gr_idx = gold_records.index(
                    max(gold_records, key=_tmp_key))
...
def agg_ins_event_level_tpfpfn_stats(
    ...
    """
    :description: Aggregate TP,FP,FN statistics for events.
    :param (*)
    :return (*)
    """
    assert len(pred_record_mat) == len(gold_record_mat)
...
def get_mcml_prf1(pred_event_types, gold_event_types, event_type_roles_list):
    """
    :description: get p r f1 measures of event type classification results.
    :param (*)
    :return (*)
    """
...
def get_ent_prf1(pred_spans_token_tuple_list, gold_spans_token_tuple_list):
    ...
    :description: 
        ...
        'entity' means the Named Entity Recognition (NER) part of measurements.
    :param (*)
    :return (*)
    """
    tot_tp_fp_fn = [0] * 3
...
def get_combination_prf1(
    ...
    """
    :description: 
        get p r f1 measures of combination results.
        combination is the combination evaluation results after the final instance generation process. Some arguments in rawCombination may be filtered out.
    :param (*)
    :return (*)
    """
    TP, FP, FN = 0, 0, 0
    for pred_record_mat, gold_record_mat in zip(
        ...
        if input_type == "event_obj":
            pred_combinations = extract_combinations_from_event_objs(
                pred_record_mat)
            gold_combinations = extract_combinations_from_event_objs(
                gold_record_mat)
        else:
...
def get_instance_prf1(pred_record_mat_list, gold_record_mat_list):
    """
    :description: 
        get p r f1 measures of instance-level results.
        one instance is recognised as True Positive (TP) if all the argument roles have filled with correct arguments.
    :param (*)
    :return (*)
    """
    TP, FP, FN = 0, 0, 0
...
def get_adj_mat_acc(pred_adj_mats, gold_adj_mats):
    """
    :description: 
        get p r f1 measures of adj_mat results.
        adj_mat means the evaluation of adjacent matrix for each document.
    :param (*)
    :return (*)
    """
...
def get_adj_mat_conn_metrics(batch_pred_adj_mats, batch_gold_adj_mats):
    """
    :description: 
        get p r f1 measures of adj_mat connection results.
        connection means the evaluation of connections between pseudo triggers and ordinary arguments.
    :param (*)
    :return (*)
    """
...
def get_trigger_identification_metrics(batch_pred_adj_mats, batch_gold_adj_mats):
    """
    :description: 
        get p r f1 measures of trigger_identification results.
        trigger means the evaluation of pseudo triggers.
    :param (*)
    :return (*)
    """
    assert len(batch_pred_adj_mats) == len(batch_gold_adj_mats)
...
def measure_event_table_filling(
    ...
    """
    :description: get all the metrics of event_table filling results.
    :param (*)
    :return (*)
    """
    """
    ...
    ent_prf1 = get_ent_prf1(pred_spans_token_tuple_list,
                            gold_spans_token_tuple_list)

    if pred_combinations and gold_combinations:
        # rawCombination is the combination evaluation results after the BK extraction without further instance generation and argument filtering.
        raw_combination_prf1 = get_combination_prf1(
            pred_combinations, gold_combinations, input_type="combination"
        )
    #  combination is the combination evaluation results after the final instance generation process.
    #  Some arguments in rawCombination may be filtered out.
    combination_prf1 = get_combination_prf1(
        pred_record_mat_list, gold_record_mat_list)

    instance_prf1 = get_instance_prf1(
        pred_record_mat_list, gold_record_mat_list)
